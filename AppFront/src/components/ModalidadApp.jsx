...
export const ModalidadApp = ({ userLog, setUserLog }) => {
    ...
    const columnSettings = {
        ...
        tipo: { label: "Descripción", type: "string", classname: "text-start", default: true }
    };
        ...
        const load = async () => {
            ...
            try {
                ...
                const [response, permission] = await Promise.all([
                    getSchedule(query.page, query.size, query.order, filtrosFinal),
                    getPermission('', '', '', `tipousuario.id:eq:${userLog?.tipousuario?.id};modulo.var:eq:rh02`)
                ]);
    ...
    const guardarFn = async (formData) => {
        ...
        if (rowAGuardar.id) {
            await updateSchedule(rowAGuardar.id, rowAGuardar);
            await AddAccess('Modificar', rowAGuardar.id, userLog, "Modalidades");
        } else {
            const nuevoModalidad = await saveSchedule(rowAGuardar);
    ...
    const handleView = async (row) => {
        await AddAccess('Visualizar', row.id, userLog, "Modalidades");
    ...
    const handleDelete = async (row) => {
        const rel = await getShift('', '', '', `tipoturno.id:eq:${row?.id}`);
    ...
    return (
        <>
            ...
            {rowAEliminar && (
                <Delete setEliminar={setRowAEliminar} title={'modalidad'} gen={false} confirmar={confirmarEliminacion} id={rowAEliminar.id} />
            )}
            {rowNoEliminar && (
                <NotDelete setNoEliminar={setRowNoEliminar} title={'modalidad'} gen={false} />
            )}
            {rowAVisualizar && (
                <SmartModal
                    open={!!rowAVisualizar}
