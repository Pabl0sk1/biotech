...
export const ComisionApp = ({ userLog, setUserLog }) => {
        ...
        const load = async () => {
            ...
            try {
                ...
                const [response, permission] = await Promise.all([
                    getCommission(query.page, query.size, query.order, filtrosFinal),
                    getPermission('', '', '', `tipousuario.id:eq:${userLog?.tipousuario?.id};modulo.var:eq:cm04`)
                ]);
    ...
    const handleView = (row) => {
        ...
        AddAccess('Visualizar', row.id, userLog, "Comisiones");
    };
