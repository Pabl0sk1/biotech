...
export const MonedaApp = ({ userLog, setUserLog }) => {
        ...
        const load = async () => {
            ...
            try {
                ...
                const [response, permission] = await Promise.all([
                    getCurrency(query.page, query.size, query.order, filtrosFinal),
                    getPermission('', '', '', `tipousuario.id:eq:${userLog?.tipousuario?.id};modulo.var:eq:gr02`)
                ]);
    ...
    const handleView = (row) => {
        ...
        AddAccess('Visualizar', row.id, userLog, "Monedas");
    };
