...
const AutoField = ({ name, value, onChange, disabled, settings, errors }) => {
    ...
    const common = {
        ...
        maxLength: size,
        className: `${type !== "checkbox" ? "modern-input-edit" : "opacity-0 position-absolute"} ${errors ? 'error' : ''}`,
    ...
    if (type === "checkbox") {
        return (
            <div className="d-flex justify-content-center align-items-center py-1">
                <label style={{ cursor: common.disabled ? 'default' : 'pointer', position: 'relative', display: 'inline-block', width: '52px', height: '28px' }}>
                    <input type="checkbox" {...common} />
                    <span className={`form-check-edit-l1 ${value ? 'bg-success' : 'bg-secondary-subtle'} ${common.disabled ? 'opacity-50' : 'opacity-100'}`}>
